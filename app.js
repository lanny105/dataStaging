...
function initParse() {   //used for operation on concon-admin
...
function changeParse() {    //used for operation on concon-live
...
getParse(function(res) {    // set the initial hashtable from concon-live to concon-admin
    for(var i = 0; i < res.length; ++i) {
        console.log(res[i].id);
        localStorage.setItem(res[i].id,res[i].id); // same
    }
});
...
function save(req, callback) {    //save a new event
    initParse();
    ...
    if(req.body.eventStartDate!='') {
        event.set("eventStartDate",new Date(req.body.eventStartDate +" UTC"));    // regard the time as UTC and store this value in database.
    }
    ...
    event.save(null, {
        success: function(event) {
            ...
            if(event.get("prodData")) {    // need to push to production
                ...
                event2 = event.clone();  // make a copy 
            ...
            success: function(event) {
                ...
                event.save(null, {
                    success: function(event) {
                        ...
                        if(req.body.prodData) {    // need to syc on the production
